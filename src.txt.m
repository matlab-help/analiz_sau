classdef tau < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function Button_9Pushed(app, event)
            ...
            U = real(pol_B);
            V = imag(pol_B);
            xlswrite(strcat(PathName,Filaname),[string('частота'), string('Re(D)'), string('Im(D)'), string('частота (загрубление)'), string('Re(D) (загрубление)'), string('Im(D) (загрубление)')],'Годограф Михайлова','A1:F1');
            xlswrite(strcat(PathName,Filaname),[imag(w)' U' V'],'Годограф Михайлова',strcat('A2:C',string(length(w))));
            k = 1;
            w1 = [];
            U1 = [];
            V1 = [];
            ...
            xlswrite(strcat(PathName,Filaname),[imag(w1)' U1' V1'],'Годограф Михайлова',strcat('D2:F',string(length(w1))));
            clear w V U w1 V1 U1
